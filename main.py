...
def generate_embeddings(request: dict):
    """Custom Web API endpoint for generating embeddings using Hugging Face models.
    
    This endpoint is used by Azure AI Search indexer skillsets when EMBEDDING_PROVIDER=hf.
    It generates embeddings for text content using the configured Hugging Face model.
    
    Request format expected by Azure AI Search WebApiSkill:
    {
        "values": [
            {
                "recordId": "unique-id",
    ...
    print(f"🧮 Generating embeddings using {SETTINGS.embedding_provider} provider")
    ...
    try:
        ...
        for value in values:
            ...
            try:
                # Generate embedding using the configured Hugging Face model
                vector = encode(text)
                ...
                print(f"✅ Generated embedding for record {record_id} (dim: {len(vector)})")
                
            except Exception as e:
                print(f"❌ Failed to generate embedding for record {record_id}: {e}")
                response_values.append({
                    "recordId": record_id,
                    "data": {},
                    "errors": [{
